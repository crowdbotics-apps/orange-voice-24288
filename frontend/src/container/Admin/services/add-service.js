    ...
    (e) => {
      ...
      if (formValues.description.length < 25) {
        setNotValid({
          error: true,
          type: 'description',
          message: 'Description must contain min 25 characters',
        });
        return;
      }
      if (formValues.description.length > 250) {
        setNotValid({
          error: true,
          type: 'description',
          message: 'Description exceed 250 characters ',
        });
        return;
      }
      ...
      if (formValues.shortDescription.length < 15) {
        setNotValid({
          error: true,
          type: 'shortDescription',
          message: 'Short description must contain 15 characters',
        });
        return;
      }
      if (formValues.shortDescription.length > 20) {
        setNotValid({
          error: true,
          type: 'shortDescription',
          message: 'Short description exceed 20 characters',
        });
        return;
      }
      if (!formValues.minQty) {
        setNotValid({
          error: true,
          type: 'minQty',
          message: 'Please provide min order qty',
        });
        return;
      }
      if (Number(formValues.minQty) === 0) {
        setNotValid({
          error: true,
          type: 'minQty',
          message: 'Minimum order qty could not be zero',
        });
        return;
      }
      if (Number(formValues.minQty) < 0) {
        setNotValid({
          error: true,
          type: 'minQty',
          message: 'Negative number not allow in min qty',
        });
        return;
      }
      if (Number(formValues.minQty) > 9999) {
        setNotValid({
          error: true,
          type: 'minQty',
          message: 'Min qty should not exceed 9999',
        });
        return;
      }
      if (!formValues.price) {
      ...
      if (Number(formValues.price) === 0) {
        setNotValid({
          error: true,
          type: 'price',
          message: 'Price could not be zero',
        });
        return;
      }
      if (Number(formValues.price) < 0) {
        setNotValid({
          error: true,
          type: 'price',
          message: 'Negative number not allow in price',
        });
        return;
      }
      if (!/^\s*-?[1-9]\d*(\.\d{1,2})?\s*$/.test(formValues.price)) {
        setNotValid({
          error: true,
          type: 'price',
          message: 'Price must be a number with upto 2 decimal places',
        });
        return;
      }
      if (Number(formValues.price) > 9999) {
        setNotValid({
          error: true,
          type: 'price',
          message: 'Price should not exceed 9999',
        });
        return;
      }
              ...
              <Form onSubmit={addService}>
                  ...
                  <Col sm="6">
                    <FormGroup>
                      <Label for="exampleSelect">
                        <span className="text-danger">*</span> Categories
                      </Label>
                      <Input
                        type="select"
                        name="select"
                ...
                <Row>
                  ...
                  <Col sm="6">
                    <FormGroup>
                      <Label>
                        <span className="text-danger">*</span> Short Description{' '}
                        <b>
                          <i> Write 15-20 Characters</i>{' '}
                        </b>{' '}
                      </Label>
                      <Input
                      ...
                      />
                      {notValid.error &&
                        notValid.type === 'shortDescription' && (
                          <label className=" ml-3 text-danger">
                            {notValid.message}
                          </label>
                        )}
                ...
                <Row>
                  <Col sm="6">
                    <FormGroup>
                      <Label>
                        <span className="text-danger">*</span> Min Order Qty{' '}
                      </Label>
                      <Input
                        ...
                        onChange={(e) =>
                          setFormValues({...formValues, minQty: e.target.value})
                        }
                      />
