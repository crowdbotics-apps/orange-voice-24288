...
function Locations() {
  ...
  const onTableChange = useCallback(
    (type, newState) => {
      if (type === 'pagination')
        dispatch(
          LocationActions.getLocations(newState?.page, undefined, search),
        );
    },
    [dispatch, search],
  );
        ...
        return (
          <div>
            <Button
              ...
              type="button"
              onClick={() =>
                dispatch(LocationActions.toggleEditLocationModal(rowIndex))
              }>
            ...
            <Button
              ...
              type="button"
              onClick={() =>
                dispatch(LocationActions.toggleDelLocationModal(rowIndex))
              }>
              <i className="fas fa-trash-alt" />
            </Button>
            <UncontrolledTooltip delay={0} target={`del-${rowIndex}`}>
              Remove
            </UncontrolledTooltip>
          </div>
        );
      ...
      <div className="content">
            ...
            <Card>
              ...
              <CardBody>
                {isProgress ? (
                  ...
                  <>
                    <Badge color="primary">{paging.count} Locations</Badge>
                    <ToolkitProvider
                      keyField={'id'}
                      data={locations}
                      columns={columns}
        ...
        {openDeleteModal && (
          <DeleteModal
            ...
            toggle={() => dispatch(LocationActions.toggleDelLocationModal())}
            isProgress={isProgress}
            delFunc={() => dispatch(LocationActions.delLocation(location?.id))}
          />
        )}
      </div>
    </>
