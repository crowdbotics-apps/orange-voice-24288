...
export class DriverEpics {
  static getDrivers(action$, state$, {ajaxGet, getRefreshToken}) {
    return action$.pipe(
      ofType(DriverTypes.GET_DRIVERS_PROG),
      switchMap(({payload}) => {
        return defer(() => {
          return ajaxGet(
            `api/v1/${state$.value.auth.user.domain}/driver/?page=${
              payload?.page
            }&offset=${(payload?.page - 1) * 10}&search=${payload.search}`,
          );
        }).pipe(
          pluck('response'),
          map((obj) => {
          ...
          catchError((err, source) => {
            if (err.status === 401) {
              ...
              return of({
                type: DriverTypes.GET_DRIVERS_FAIL,
                payload: {
                  err,
  ...
  static getDriverHistory(action$, state$, {ajaxGet, getRefreshToken}) {
    return action$.pipe(
      ofType(DriverTypes.GET_DRIVER_HISTORY_PROG),
      switchMap(({payload}) => {
        return defer(() => {
          return ajaxGet(
            `api/v1/${state$.value.auth.user.domain}/driver/${
              payload.driverId
            }/order?page=${payload?.page}&offset=${
              (payload?.page - 1) * 10
          ...
          catchError((err, source) => {
            if (err.status === 401) {
              ...
              toast.error(message ? message : ErrorMsg);
              return of({
                type: DriverTypes.GET_DRIVER_HISTORY_FAIL,
                payload: {
                  err,
                  message: message ? message : ErrorMsg,
                  status: err?.status,
        ...
        return defer(() => {
          ...
          catchError((err, source) => {
            if (err.status === 401) {
              ...
              toast.error(message ? message : ErrorMsg);
              return of({
                type: DriverTypes.ADD_DRIVER_FAIL,
                payload: {
                  err,
                  message: message ? message : ErrorMsg,
                  status: err?.status,
  ...
  static editDriver(action$, state$, {ajaxPut, getRefreshToken, history}) {
    return action$.pipe(
      ofType(DriverTypes.EDIT_DRIVER_PROG),
      switchMap(({payload}) => {
        return defer(() => {
          return ajaxPut(
          ...
          catchError((err, source) => {
            if (err.status === 401) {
              ...
              toast.error(message ? message : ErrorMsg);
              return of({
                type: DriverTypes.EDIT_DRIVER_FAIL,
                payload: {
                  err,
                  message: message ? message : ErrorMsg,
                  status: err?.status,
        ...
        return defer(() => {
          ...
          catchError((err, source) => {
            if (err.status === 401) {
              ...
              toast.error(message ? message : ErrorMsg);
              return of({
                type: DriverTypes.DEL_DRIVER_FAIL,
                payload: {
                  err,
                  message: message ? message : ErrorMsg,
                  status: err?.status,
